...
class TaskInference(BaseModule):
    ...
    def training_step(
        self,
        encoder_context: DictReplayBufferSamples,
        decoder_context: DictReplayBufferSamples,
    ):
        config = self.config.training
        batch_size = encoder_context.rewards.shape[0]

        decoder_action = decoder_context.actions
        decoder_state = decoder_context.observations["observation"]
        decoder_next_state = decoder_context.next_observations["observation"]
        decoder_reward = decoder_context.rewards
        true_task = decoder_context.next_observations["task"]
        ...
        if config.alpha_kl_z_query is not None:
            y_distribution_query, z_distributions_query, _ = self.encoder.encode(
                EncoderInput(
                    obs=decoder_context.observations["observation"],
                    action=decoder_context.actions,
                    next_obs=decoder_context.next_observations["observation"],
        ...
        # Overall ELBO
        if not config.component_constraint_learning:
            elbo = th.sum(
        ...
        # Component-constraint_learing
            ...
            temp = th.zeros_like(y_probs)
            true_task_multiplier = temp.scatter(1, true_task.unsqueeze(1).long(), 1)
            loss_nll = th.nn.NLLLoss(reduction="none")
            target_label = true_task.long()
            elbo = th.sum(
                th.sum(th.mul(true_task_multiplier, (-1) * nll_px - config.alpha_kl_z * kl_qz_pz), dim=-1)
                - config.beta_kl_y * loss_nll(th.log(y_probs), target_label)
            )
