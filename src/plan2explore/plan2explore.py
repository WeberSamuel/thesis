...
class Plan2Explore(StateAwareOffPolicyAlgorithm):
    ...
    def  _setup_model(self, parent_algorithm: OffPolicyAlgorithm | None = None) -> None:
        if isinstance(parent_algorithm, CEMRL):
            self.policy_kwargs["encoder"] = parent_algorithm.policy.task_inference.encoder
            self.policy_kwargs["ensemble"] = parent_algorithm.policy.task_inference.decoder
    ...
    def train(self, gradient_steps: int, batch_size: int) -> None:
        ...
        for _ in range(gradient_steps):
            if isinstance(self.replay_buffer, CEMRLReplayBuffer):
                ...
                obs = dec_input.observations
                next_obs = dec_input.next_observations
                _, z, *_ = self.replay_buffer.task_inference(
                    EncoderInput(
                        obs=enc_input.observations,
                        action=enc_input.actions,
                        reward=enc_input.rewards,
                        next_obs=enc_input.next_observations,
                    )
                )
            ...
            pred_next_obs, pred_reward = self.policy.ensemble(obs, actions, None, z, return_raw=True)
